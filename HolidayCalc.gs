...
function testDate() {
  var testDate = new Date(2021, 0, 11 ,12,0,0);
  Logger.log(testDate);
...
function checkHoliday(testDate) {
  ...
  switch (easterdifference) {
    case -52: holiday = {name:"Giovedì Grasso", psalm:"B", holy:false, tempo:"O" ,special:true, color:"V" };break;
    case -47: holiday = {name:"Martedì Grasso", psalm:"B", holy:false, tempo:"O" ,special:true, color:"V" };break;
    ...
    case -41: holiday = {name:"Sabato Grasso (Ambrosiano)", psalm:"B", holy:true, tempo:"Q" ,special:true, color:"V" };break;
  ...
  switch (adventdifference) {
    ...
    case -343:  holiday = {name:"Battesimo del Signore", psalm:"L", holy:true, tempo:"N" ,special:true, color:"W" };break;
  }
  if (holiday) {Logger.log(holiday.name); return JSON.stringify(holiday);}
  
  // Search for Sacra Famiglia
  if (isSacraFamiglia(testDate)) {holiday = {name:"Sacra Famiglia", psalm:"L", holy:true, tempo:"" ,special:true, color:"" }}
  ...
  switch (stringCheck) {
    ...
    case "1607": holiday = {name:"Assunzione", psalm:"G", holy:true, tempo:"O" ,special:true, color:"A" };break;
    ...
    case "2611": holiday = {name:"S. Stefano", psalm:"L", holy:true, tempo:"N" ,special:true, color:"R" };break;

    case "3106": holiday = {name:"S. Ignazio di Loyola", psalm:"B", holy:false, tempo:"O" ,special:true, color:"V" };break;
    case "2905": holiday = {name:"Santi Pietro e Paolo", psalm:"B", holy:false, tempo:"O" ,special:true, color:"R" };break;
    case "3111": holiday = {name:"S. Silvestro", psalm:"L", holy:false, tempo:"N" ,special:true, color:"R" };break;
    case "1902": holiday = {name:"S. Giuseppe", psalm:"B", holy:false, tempo:"O" ,special:true, color:"V" };break;
  }
  if (holiday) {Logger.log(holiday.name); return JSON.stringify(holiday);}
  ...
  if (easterdifference > 0 && easterdifference <48) {holiday.color="W";holiday.tempo = "P";}
  if (adventdifference > -28 && adventdifference <6) {holiday.color="V";holiday.tempo = "A";}
  if (adventdifference > -359 && adventdifference <-342) {holiday.color="W";holiday.tempo = "N";}
